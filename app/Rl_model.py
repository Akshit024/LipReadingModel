...
class Temporal_difference_off_policy(object):
    ...
    def create_variables(self):
        ...
        with tf.name_scope("predict_actions"):
            ...
            with tf.variable_scope("q_network"):
                self.q_outputs, self.output_state = self.q_network(self.states)
        ...
        with tf.name_scope("estimate_future_rewards"):
            self.next_states = tf.placeholder(dtype=tf.float64, shape=(None, 74, self.state_dim), name="next_states")
            self.next_state_mask = tf.placeholder(dtype=tf.float64, shape=(None,), name="next_state_mask")
            ...
            if self.double_q_learning:
                ...
                with tf.variable_scope("target_network"):
                    self.target_outputs, _ = self.q_network(self.next_states) * tf.cast(self.action_selection_mask, tf.float64)
                ...
                with tf.variable_scope("target_network"):
                    self.target_outputs, _ = self.q_network(self.next_states)
            ...
            self.rewards = tf.placeholder(dtype=tf.float64, shape=(None,), name="rewards")
        ...
        with tf.name_scope("compute_temporal_differences"):
            ...
            self.action_mask = tf.placeholder(dtype=tf.float64, shape=(None, self.num_actions), name="action_mask")
    ...
    def eGreedyAction(self, states_, explore=True):
    ...
    def updateModel(self):
        ...
        states = np.zeros((self.batch_size, 74, self.state_dim))
        rewards = np.zeros((self.batch_size,))
        action_mask = np.zeros((self.batch_size, self.num_actions))
        next_states = np.zeros((self.batch_size, 74, self.state_dim))
